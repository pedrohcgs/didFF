...
library(didFF) #load the didFF package
library(tidyverse) 
library(dplyr)
...
# Create dataset (not run)
mw_df_2007_2015 <- 
  mw_df %>%
  dplyr::filter((start_t <= year) & (year <= end_t)) %>% # filter out data outside the time window of interest
  dplyr::group_by(statenum) %>%
  dplyr::mutate(group = max(treated > 0)*2015) %>% # Create group variable that take value end_t if state was ever treated in this period, and 0 otherwise
  dplyr::filter((year == start_t) | (year == end_t)) %>%#filter data back to pre and post periods only
  dplyr::group_by(statenum, wage) %>%
  dplyr::mutate(state_bin_id = cur_group_id() )

# Run the test (not run)
